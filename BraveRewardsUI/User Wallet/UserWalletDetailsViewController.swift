...
final class UserWalletDetailsViewController: UIViewController {
  ...
  var disconnectedWallet: () -> Void
  
  init(state: RewardsState, wallet: ExternalWallet, disconnectedWallet: @escaping () -> Void) {
    self.state = state
    self.wallet = wallet
    self.disconnectedWallet = disconnectedWallet
...
extension UserWalletDetailsViewController {
...
private class UserWalletSummarySectionView: SettingsSectionView {
  ...
  let verifiedLabel = UILabel().then {
    ...
    $0.font = .systemFont(ofSize: 15.0)
    $0.textColor = BraveUX.upholdGreen
    $0.isHidden = true
